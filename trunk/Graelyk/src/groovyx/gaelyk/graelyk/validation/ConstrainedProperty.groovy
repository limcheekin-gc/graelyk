...
import groovyx.gaelyk.graelyk.exception.ConstraintException
...
public class ConstrainedProperty   
{
    ...
    static final String DEFAULT_NOT_UNIQUE_MESSAGE_CODE = "default.not.unique.message";
    ...
    protected static final ResourceBundle bundle = ResourceBundle.getBundle("groovyx.gaelyk.graelyk.validation.DefaultErrorMessages", Locale.getDefault(), Thread.currentThread().getContextClassLoader());
    ...
    // simple constraints
    private boolean display = true; // whether the property should be displayed for display
    ...
    public boolean isEmail() {
        if(isNotValidStringType() && isNotEmailType()) {
	...
	private boolean isNotEmailType() {
		return propertyType.isAssignableFrom(Email);
	}
    ...
    public boolean isUnique() {
        return this.appliedConstraints.containsKey( UNIQUE_CONSTRAINT );
    }
    ...
    public void setUnique(boolean unique) {
        ...
        if(unique) {
            ...
            if(c != null) {
                this.appliedConstraints.remove( UNIQUE_CONSTRAINT );
            }
        }
    }
