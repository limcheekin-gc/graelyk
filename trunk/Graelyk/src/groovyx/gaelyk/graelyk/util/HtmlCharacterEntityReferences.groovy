	...
	public HtmlCharacterEntityReferences() {
		...
		try {
			try {
				...
				{
					System.err.println("Error trying to close InputStream for HtmlCharacterReferences. " + gaex.toString())
				}
			}
		}
		...
		while (keys.hasMoreElements()) {
			...
			/*
			org.springframework.util.Assert.isTrue((referredChar < 1000 || (referredChar >= 8000 && referredChar < 10000)),
			...
			*/
			...
			this.characterToEntityReferenceMap[index] = REFERENCE_START.toString() + reference + REFERENCE_END.toString();
