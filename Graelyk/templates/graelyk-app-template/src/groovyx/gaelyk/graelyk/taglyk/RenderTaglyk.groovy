...
//Originally from package org.codehaus.groovy.grails.plugins.web.taglib
 ...
 * @since 17-Jan-2006
 */
class RenderTaglyk // implements com.opensymphony.module.sitemesh.RequestConstants
{

	static void g_paginate(script, attrs){script.out << script.paginate(attrs)}
	static void g_sortableColumn(script, attrs){script.out << script.sortableColumn(attrs)}
...
/*	Todo: figure out what this does & if I need to re-enable it
    protected getPage() {
    	return request[PAGE]
    }
*/
...
/*
    def pageProperty = { attrs ->
            if(!attrs.name) {
	            throw new TaglykException("Tag [pageProperty] is missing required attribute [name]")
            }
...
*/
	 ...
	 */
	static paginate(script, attrs)
	{
		def writer = new StringWriter()
		def params = script.params
        if(attrs.total == null)
		{
            throw new TaglykException("Tag [paginate] is missing required attribute [total]")
		}
                                    
		def messageSource = script.messageBundle
		def locale = script.userLocale

		def total = attrs.int('total') ?: 0
		//def action = (attrs.action ? attrs.action : (params.action ? params.action : "list"))
		def action = (attrs.action ? attrs.action : (script.actionName ?: "list"))
		...
		if(currentstep > firststep) {
            ...
            linkParams.offset = offset - max
            writer << script.link(linkTagAttrs.clone()) {
				(attrs.prev ? attrs.prev : script.message('paginate.prev', [], script.message('default.paginate.prev', [], 'Previous', locale), locale))
			 }
		}
		...
		if(steps && laststep > firststep) {
			...
			if(beginstep > firststep) {
				linkParams.offset = 0
				writer << script.link(linkTagAttrs.clone()) {firststep.toString()}
			...
			(beginstep..endstep).each { i ->
				if(currentstep == i) {
					...
					writer << script.link(linkTagAttrs.clone()) {i.toString()}
				}
			}
			...
			if(endstep < laststep) {
				...
				writer << script.link(linkTagAttrs.clone()) { laststep.toString() }
			}
		}
		...
		if(currentstep < laststep) {
			...
			linkParams.offset = offset + max
			writer << script.link(linkTagAttrs.clone()) {
				(attrs.next ? attrs.next : script.message('paginate.next', [], script.message('default.paginate.next', [], 'Next', locale), locale))
			}
		}
	 ...
	 * defaultOrder (optional) - default order for the property; choose between "" (for ascending - default if not provided) and "-" (for descending)
	 ...
	 * <g:sortableColumn property="releaseDate" defaultOrder="-" title="Release Date" />
	 * <g:sortableColumn property="releaseDate" defaultOrder="-" title="Release Date" titleKey="book.releaseDate" />
	 */
	 
	static sortableColumn(script, attrs)
	{
		def writer = new StringBuffer()
		if(!attrs.property)
		{
			throw new TaglykException("Tag [sortableColumn] is missing required attribute [property]")
		}

		if(!attrs.title && !attrs.titleKey)
		{
			throw new TaglykException("Tag [sortableColumn] is missing required attribute [title] or [titleKey]")
		}
		...
		attrs.each { k, v ->
			writer << "${k}=\"${v.encodeAs("HTML")}\" "
		}
		writer << ">" + script.link(action:action, params:linkParams){ title } + "</th>"
...
/*
    def render = { attrs, body ->
        if(!groovyPagesTemplateEngine) throw new IllegalStateException("Property [groovyPagesTemplateEngine] must be set!")
        if(!attrs.template)
            throw new TaglykException("Tag [render] is missing required attribute [template]")
        ...
        if(!t) {
            throw new TaglykException("Template not found for name [$attrs.template] and path [$uri]")            
        }
        else {
