...
import groovyx.gaelyk.graelyk.util.FileUpload
...
save = {
	...
	if(${propertyName}.validateAndSave())
	{
		//Validation succeeds
...
show = {
	view = "show"
	def ${propertyName} = ${domainClassName}.fetch(params.id).setCallingScript(this)
	//Todo: verify user has authority to view this object
...
update = {
	...
	def ${propertyName} = ${domainClassName}.fetch(params.id).setCallingScript(this) << params
...
delete = {
	...
	//flash.message = "\${message(code: 'default.not.deleted.message', args: [message(code: '${domainClassPropertyName}.label', default: '${domainClassName}'), params.id])}"
	${domainClassName}.destroy(params.id)
	flash.message = "\${message(code: 'default.deleted.message', args: [message(code: '${domainClassPropertyName}.label', default: '${domainClassName}'), params.id])}"
	list()
}

deleteMultiple = {
	if(params.id instanceof String){params.id = [params.id] as String[]}
...
/*
deleteAll = {
	def keyList = ${domainClassName}.searchKeys() //Gets the keys for all objects
	dao.destroy(keyList) //Deletes all the objects using the list of keys
	list()
}
*/
